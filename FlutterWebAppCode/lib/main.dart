...
class MyApp extends StatelessWidget {
  ...
  const MyApp({super.key}); //constructor, instantiating the myApp class

  //bool signedin=false;
  //bool isSignedIn() { //this function is used to dynamically adjust the current authentication state.
    //var authorized = FirebaseAuth.instance.authStateChanges();//value in realtime that shows currentState of authentication

    //authorized.listen((User? currentUser) { //listens to any changes in the User object(a similar listener implementation is used for the realtime values)
      //if(currentUser==null) {//checks if the currentuser exists. If it does not exists, this means the user is signed out.
        //signedin= false;
      //}
      //else { //If it does exist, this means that the user is signed in.
        //signedin=true;
      //}
  ...
  Widget build(BuildContext context) {
    return MaterialApp(debugShowCheckedModeBanner : false,
      ...
      home:  //theTab Controller controls, what is going on.
      
      //isSignedIn() ? //if the user is signed in, the home page will be shown. If the user is NOT signed in, the signinscreen will be shown.
      ...
      //SignInScreen(
        ...
        //],
        //actions:[
          //AuthStateChangeAction<SignedIn>((state) {
            //signedin=true;
          //})
        //]
      //navigators
      
      //DefaultTabController(length: 3, child: TopTabs())//length of 3 means there are 3 tabs. TopTabs is the custom made class used for the tabs. This class is stateful to ensure, that the state changes of other tabs remain, between toggling of tabs.
      SignInScreen1()//the sign in screen will be the default beginning of the webapp.
    );
  }
}
//final GlobalKey<_MyHomePageState> key1 = GlobalKey<_MyHomePageState>();//this is not a const constructor, as it depends heavily on the functions/variables in the home page. That is why global key is used here.
...
class _MyHomePageState extends State<MyHomePage> with AutomaticKeepAliveClientMixin  { //The AutomaticKeepAliveClient is used to allow for the tab state to be maintained when moving to a different tab. This related to the tabs
  ...
  //.onValue.listen((DatabaseEvent theEvent) {
    //final data= event.snapshot.value;
...
//origpH="HomePage";//represents original pH. Note: in the database pH is in terms of strings due to the to_string method in the external C++ program
//this is specified outside scope of this function as it is a state variable.
...
String showpHData() { //This is a function in the class that is used to display realtime values. It returns a pH of type String.
    DatabaseReference myDatabase=FirebaseDatabase.instance.ref('data'); //This is the instance of the database. This code will read/write from this instance. 'data' is the name of the data stored in the database.
    //In the .json format, the database is structured with header 'data', unique ID tags(which are randomized combinations of letters and numbers), and the associated pH string value with each ID
    ...
    myDatabase.onChildAdded.listen((DatabaseEvent event)   { //the database listens if any child is added(aka, a new ID is added). This is conditioned on an 'event' which will be specified in the body.
      ...
      final pH = event.snapshot.child('pHvalue').value; //the new pH is the upcoming(realtime) event's child's value. 'pHvalue' is the property that specifies pH value.
        
      setState(() { //adjusts value of origpH dynamically
      if(pH!=null) {//checks to ensure that final pH is not null. This check is placed here, as when adjusting flouride values, it causes pH to be null. This check ensures that the value displayed on the screen will NEVER be null.
  ...
  return origpH;//origpH is returned, and updated in realtime on Flutter.
}
...
String showlimedispensionData() { //This is a function in the class that is used to display realtime values. It returns a limedispension value of type String.
    ...
    myDatabase.onChildAdded.listen((DatabaseEvent event)   { //the database listens if any child is added(aka, a new ID is added). This is conditioned on an 'event' which will be specified in the body.
        ...
        final limedispension=event.snapshot.child('LimeDispensionRate').value; //the new lime dispension is the upcoming(realtime) event's child's value. 'limedispensionValue' is the property that specifies limedispension value.
        ...
        origlimedispension = limedispension.toString(); //ensures origpH is in terms of string of dynamic pH.
      }
  });
  }
});
  return origlimedispension;//origlimedispension is returned, and updated in realtime on Flutter.
}
...
String showRealTime() {
  DatabaseReference myDatabase=FirebaseDatabase.instance.ref('data'); //This is the instance of the database. This code will read/write from this instance. 'data' is the name of the data stored in the database.
    //In the .json format, the database is structured with header 'data', unique ID tags(which are randomized combinations of letters and numbers), and the associated flouride string value with each ID
    //each value represents a flouride string value, and each ID is a 'child' of data.
    //the purpose of this is to listen to changes in the children(if different children values are added), and evantually display(print) these changes in realtime to the Flutter App.
    myDatabase.onChildAdded.listen((DatabaseEvent event)   { //the database listens if any child is added(aka, a new ID is added). This is conditioned on an 'event' which will be specified in the body.
  ...
  //print(origTime);
  return origTime;//origTime is returned, and updated in realtime on Flutter.
}
...
List<DataRow> theData3=[];//this represents the list of the row to be added.

final myStopWatch=Stopwatch(); //this represents the instantiation of the StopWatch. This stopwatch will be used for the creation of the graphs
...
void initState() { //this is like an allocating memory function. It creates an instance of an object
  ...
  starttheWatch(0); //this Starts the watch. Each time a new object is created, theWatch is started.
...
//This is also used as a point of interest in the code in the sense that the user can use this to determine when they want to save a point of interest.


bool light=true; //this is for the switch. light=true means on(play), while light=false means off(pause)
...
//This function essentially connects the frontend switch to the backend code.
  ...
  if(!light) { //if the light is off, this means that the switch is in the off state. This means that the watch should be paused
    ...
    double currentTimePlayednum=double.parse(currentTimePlayed.millisecondsSinceEpoch.toString());//this obtains the currentTime that was played in terms of a double.
    ...
    //this cool off ensures data crashing does not occur.
    ...
    else {//If there is button spamming(switch spamming), this message will be displayed to alert the user to calm down on the button spamming.
    ...
    setState(() {
      ...
      previousTimeplayed=currentTimePlayednum;//this is used to set the current time as the previous time
    });
...
void sliderState(double val2) {//Need to look into how this changes, when adjusting the slider.
  ...
  setState(() { //dynamically sets the state of samplingTime to val2. Val2 is a dynamically changing parameter, which represents the user inputted sampling time.
    samplingTime=val2;//similar to the switch, the slider's sampling Time value is also changed.
  });
  ...
  myStopWatch.start(); //the stopwatch is now started for the graph. The stopwatch upcounts in time
      ...
      setState(() { //sets dynamically changing values in time. Ensures changes are reflected in realtime on webapp.
        if(count==0) { //this is an if statement specially made for t=0s(if startTime=0s). Other values of startime are added in the pausethewatch function.
          ...
          theData.add(FlSpot(startTime.toDouble(), myDouble)); //adds the pH data at time t=0s(starting time). The paused value will be added in the pausetheWatch function.
          //(which makes sense in the sense that the user wants to take an instantaneous measurement. That is the purpose of this.)
        }
        count++; //since the first datapoint(at t=0s) is added, it should not be added again.

        double currentTime=myStopWatch.elapsedMilliseconds/1000.toDouble(); //current stopwatch time. .toDouble()'s purpose is to convert the default time in int to a double

        theData.add(FlSpot(currentTime, double.parse(showpHData()))); 
        ...
        if (theData.length > 20) { //limits the number of points to be 20 at once.
          theData.removeAt(0);// Removes the first point to ensure enough points fit on the graph. This dynamically changes the size of the graph.
        }
      });
    });
}
...
void resettheWatch() {//resets time keeping back to 0, and goes from there. This is called after pressing the RESET BUTTON.
  forPeriodic?.cancel(); //cancels the timer from the previous run. ? is used here to mainly avoid errors.
  myStopWatch.stop(); //stops the watch temporarily.
  setState(() {
  ...
  for(int j=theData2.length-1;j>=0;j--) { 
  ...
  for(int k=theData3.length-1;k>=0;k--) {
    theData3.removeAt(k);//this removes the data table entry.
  }
  ...
  myStopWatch.reset(); //resets the watch back to 0 temporarily.
  ...
  starttheWatch(0); //starts theWatch from the beginning.
}
...
void pausetheWatch() { //pauses the timer/stopwatch. If the light is off, that means that it is paused. 
  //setState(() {
  forPeriodic?.cancel(); //cancels the timer from the previous run. ? is used here to mainly avoid errors(as it is not known whether or not the object is null). This mainly canceled to ensure the sampling is stopped.
  ...
  setState(() {//setState is used to dynamically change the value in realtime on the webapp
    if(theData2.isEmpty || (pause>=theData2.last.x+0.3)) //if the paused data does not contain anything(first paused datapoint) or if the current paused datapoint is at least 0.3s more than the previous, show this on the graph. This 0.3s buffer is made to avoid any spamming errors(to avoid adding excess datapoints overlayed).
    ...
    if(theData.isEmpty) {//checks if the regular data is empty.
      theData.add(FlSpot(0, double.parse(showpHData()))); //adds 0 if the paused point occurs between 0 and sampling time. This resolves an edge case bug.
    }
    ...
    //This is a storage datapoint for the user to be able to save for later. It will always be stored, but it will only conditionally be displayed
  ...
  setState(() {//ensures values are properly changed on GUI.
    ...
    theData3.add(DataRow(cells:[DataCell(Text(showpHData())), DataCell(Text(DateTime.now().toString()))]));//this adds a pasued data value to the list.
    //}
  });
...
void playtheWatch()  {
  ...
  myStopWatch2.stop(); //finds the amount of time elapsed during pausing of data value intake.
  ...
  pause=0; //pause is set back to 0, as the paused state is over
...
@override
  void dispose() { //this is like a deleting memory function.
    myStopWatch.stop(); //stops the currently running stopWatch
    myStopWatch.reset(); //resets the watch back to 0 for later use.
    forPeriodic?.cancel(); //Cancels the timer. ? is used bc it may or may not be null(there is an error without the question mark)
    myStopWatch2.stop(); //stops the currently running stopWatch
    myStopWatch2.reset(); //resets the watch back to 0 for later use.
    super.dispose(); //Destroys the built instance. The destroy is only done at the end, as that is when all of the memory based objects(stopwatches/timers) are disabled/stopped/cancelled.
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(//framework for building this webapp. This is going to especially used as the body of the webapp.
      ...
      //),
        ...
        body: Stack( //stack is used here for the sake of overlaying widgets, and ensuring that the widgets can all be placed together in a row

        children: <Widget>[
          Padding(padding: EdgeInsets.only(left:5, ),//this padding gives space to the left.
          child:Row( //row is used for the: graph, reset button, play/pause, sampling time slider, and user inputted flouride(this flouride input posiiton may change)
        children: <Widget>[
           SizedBox(//the sized box will be used, to restrict dimensions.
          //alignment:Alignment.topLeft, //aligns text based on different settings.
          
          width: 1000, height: 635, 
          //the width and height represents the size of the graph itself.
          //child: Stack(
            child:Padding(padding: EdgeInsets.only(top:3.5),child:LineChart(LineChartData(
            gridData: FlGridData(show: true),  
            titlesData: FlTitlesData(//declares instance of LineChart
            topTitles: AxisTitles( //specifies settings of topTitle
              //axisNameWidget: Text('pH Value vs Time(seconds)'), //this is the title of the graph
            ...
            bottomTitles: AxisTitles(
              axisNameSize:16.5,//this is used to ensure the size of the axis is proper and not overlapping.
              axisNameWidget: Text('Time(seconds)'),//the bottomAxis(x-axis) will be declared as the time(in seconds)
              sideTitles: SideTitles(showTitles: true,)
            ),
            ...
            borderData: FlBorderData(show: true), //shows border around the chart
            lineBarsData: [ //this is done to obtain the value's points(creates vertical lines for the user to view points).
              LineChartBarData(
            ...
            ElevatedButton( //represents the reset button, this a button the user can always press
  ...
  Slider(
    ...
    Positioned( //this positions text in specific places
      top:40, //40 pixels below the very top
    ...
    Positioned(bottom:70, right: 100, child: 
    SizedBox(height:225, 
    //width: 100, 
    child:
...
class TablePage extends StatefulWidget {//tablepage does change with state, it is NOT stateless(static)
  //throughout this code, setState will be used, which is a property of stateful widgets to enable changes on the graphical user interface.
  const TablePage({super.key, required this.title2});//tablepage constructor. 
  
  
  final String title2;//title is a property of this class, this may be adjusted later
  //title="HelloMate";
  @override //override is used to create/destroy instances(will also be seen in the graph code). In this case, it creates a new instance of the home page state.
  State<TablePage> createState() => _TablePageState();//ensures state of homepage is able to change
}
...
enum SelectedVal {
  ...
  //flouride('Flouride Value'),//represents choice description if flouride
  ...
  //unmodifiable list is used here to ensure that the list does not change. Same logic as to static final, as the descriptions/values in this dropdown list will not change in name
...
bool isValid=false; //instantiates an indicator variable that is used to check for user input
int count2=0; //this is an indicator to obtain the number of variables checked
void readInandWriteFlouride(String inputFlouride) async { //inputFlouride is the value coming in from the Flutter webapp. async is used here to ensure it is properly added.
  isValid=false;//false until proven if true
  ...
  count2++;//each time a new flouride value is being interpreted, the amount of values interpreted increases.
  });
  

  if(isValid) { //only sends/invokes database if the string value has the capability to convert to a double.
  DatabaseReference myDatabase=FirebaseDatabase.instance.ref('data'); //This is an instance of the database.
  //Unlike the displaying of pH and lime dispension values, this will be read in.
  await myDatabase.update({ //this updates the values in .json format. 
    "Fluoride Data(ppm) added at ": DateTime.now().toString(),//this represents the currentDate and Time(which will be sent to Firebase as a String)
    "Fluoride Data(ppm) value ": inputFlouride//this is the Flouride value that the user gave in.
...
String returnLabel() {//this returns the label in the textbox
  if(isValid||count2==0) {//CHECKS if this is before the first value being inputted, or, if the value is valid. Shows Flouride Value(ppm). If the first value that the user puts in is invalid, it is not sent to firebase.
...
//pH, lime dispension are handled identically, however flouride will be handled differently.
...
void numTableEntries(String newNum) {
  ...
  if(isValid2) {//if the value is able to convert to a nonzero integer...
    setState(() {
      numEntries=int.parse(newNum);//the state of the current number of entries is set to the integer version of the number of rows.
    });
  }
...
//For flouride(due to lack of data), the timegap is limited to only to the past minute at most for now(a list will be made that is going to coninously adjust flouride values)
...
//Things remaining that need to get done: time implementation, and flouride implementation
//Time implementation plan:
//The time implementation will use the last datapoint as the reference. If the user types in last minute, it will take all the datapoints from the last minute
//and with all the datapoints of the last minute, it will use the number of entries as a reference point
//With the number of entries, the time gap between the most recent entry and the entry from a long time ago will be considered.
...
bool returninRange(DateTime time2, DateTime time1) {//this takes in 2 datetime values
  if(currentChoice2!.label=="Last Second") {//if the user wants to see values from the last second
    return ((time2.difference(time1))<=Duration(seconds: 1));//Duration is used here rather than in.... due to accuracy concerns.
  }
  ...
  return true;//if the user has not entered in anything
}

String returnErrorMessageTime() {//this function returns error message time based on time settings the user placed
  if(currentChoice2!.label=="Last Second") {
    return "one last second";
  }

  if(currentChoice2!.label=="Last Minute") {
    return "one minute";
  }
  if(currentChoice2!.label=="Last Hour") {
    return "one hour";
  }
  if(currentChoice2!.label=="Last Day") {
    return "one day";
  }
  if(currentChoice2!.label=="Last Week") {
    return "one week";
  }
  ...
  //This is checked to ensure conversion from database to myMap1 is possible.
      ...
      if(displayVal()=="Lime Dispension Rate") {//this checks if the user choice is lime dispension.
        ...
        theRows.add(DataRow(cells: [//this adds a row after going through that key-value pair
          DataCell(Text(valtoDisplay)),//prints the valtoDisplay value in the first column of the data table. Note this implementation works for pH/lime dispension. It evantually needs to be more generalized for flouride.
...
/*
      ...
      if(key=="Fluoride Data(ppm) added at ") {//this checks if this is the flouride data timestamp.
        if(flouridetimelist.isEmpty||!flouridetimelist.contains(val.toString())) {
          flouridetimelist.add(val.toString());
        }
      }

      if(key=="Fluoride Data(ppm) value ") {//this checks if this is the flouride data timestamp.
        if(flouridelist.isEmpty||!flouridelist.contains(val.toString())) {
          flouridelist.add(val.toString());
        }

        if(flouridetimelist.length==flouridelist.length){//this check is done to ensure the lengths of both list are equivalent before setting the state of this.
      setState(() {
        if(displayVal()=="Flouride Value") {
         theflourideList.add(DataRow(cells: [//this adds a row after going through that key-value pair
          DataCell(Text((flouridelist.last))),//prints the first column value
          DataCell(Text((flouridetimelist.last))),//prints the associated date/time in the second column of the data table.
        ]));
        //for(int i=0;i<theflourideList.length;i++) {
          //theRows.add(theflourideList.elementAt(i));//this adds these rows to the regular list.
        //}
        theRows=theflourideList;
        }
      });
      }
      ...
      */
      ...
      //else {//this begins the flouride initialization logic. This is used for the display of flouride values to the table.
  ...
  //On April 7th 2025, the database length is 5465 entries.
        ...
        errorMessage="You asked for more entries than are possible for the associated value, only $databaselength entries are shown";
...
if(currentChoice2!.label=="Last Second" || currentChoice2!.label=="Last Minute" || currentChoice2!.label=="Last Hour" || currentChoice2!.label=="Last Day" || currentChoice2!.label=="Last Week"
  ...
  myMap1.forEach((key,val) {//this iterates through the values that were transferred
      ...
      //val has 3 propertyVals: pH, lime dispension, and timestamp.
        ...
        if(propertyKey=="Created at") {//this checks if the indicator is "Created at". "Created at" is the indicator for time values.
          String actualString2 = lastVal.toString().replaceAll(RegExp(r'\s+'), ' ');//this normalizes the string to ensure there is only one space between each entry of the times. 
          //This represents the last time value in the database
          String actualString1 = propertyVal.toString().replaceAll(RegExp(r'\s+'), ' ');//this does the same as above. This is the currentString. This represents the currentTime in the datbase
  ...
  //The logic here should show all values of amount within that specific time duration
        ...
        errorMessage="You asked for more entries than are possible for the associated value for the past ${returnErrorMessageTime()}, only $amount entries are shown";
...
/*    
  ...
  */
    ...
    for(int i=0;i<numEntries;i++) {//this traverses through the number of user entries.
      ...
      bool valofInterest=false;
      String pHfound="";
      String limedispfound="";
      ...
      wasused.add(closestTime);//this adds the closest time to the list of used timestamps.
...
//print(firstVal);
 ...
 // }
    ...
    //LinkedHashMap<dynamic, dynamic> myMap=(currSnapshot.value as LinkedHashMap<dynamic, dynamic>); 
    
    //Object? myMap=currSnapshot.value;
    //String myMap=currSnapshot.value.toString(); 
    //print(myMap);
    //as Map<dynamic, dynamic>;

    //Map<String, String> myMap2=Map<String, String>.from(myMap);//this represents a typecast

    // as LinkedHashMap<Object?, Object?>;
    //print(currSnapshot.value.runtimeType);
    //print(myMap.runtimeType);
    //Map<String, String> myMap={};//this represents the snapshot's value being filtered into an entire String map
    
    //currSnapshot.value?.forEach((key, value)

    
    
    
    //this map represents key-value pairs. String is used here as the .json has EVERYTHING in a string format.
    
    

    //It is important to note that the key value for flouride is different from those for pH/lime dispension.
    
    /*
    setState(() {
      theRows=[];//this reinitializes theRowslist to be empty at every run.
      myMap2.forEach((key, value)//this provides a way to iterate through the map. This is identical in logic to a foreach Loop in a general program
      //it traverses through each key one by one looking at its associated values.
      //key is the associated key in Firebase, while value represents the value of that key. A filtration mechanism(by value[...] will be used to obtain the value of interest.)
      {
      theRows.add(//this adds a new datarow to the list of rows
        DataRow(
        cells: [//this represents the cells for this row(this will be adjusted later.)
          DataCell(Text(key)),//this is the firebase key
          DataCell(Text(value))
...
void dispose() {//destroys the current instance. This is done(similarly to the realtime graphs) to avoid any memory leaks.
  ...
  (
    ...
    Column(children: <Widget>[//this column is for the Flouride Input Field and its associated title
  ...
  child:TextField(
    ...
    maxLength: 9, //max amount of characters
  ...
  //)
    ...
    Padding(padding: EdgeInsets.only(left:40, top: 10, bottom: 10), child:Text("Choose the value that you would like to observe in the Data Table: ",style: TextStyle(fontSize: 17, color: Colors.purple),)),//this represents the title. Padding is placed to ensure that it is placed nicely into the UI
    SizedBox(width:230, 
      child:
      DropdownMenu<SelectedVal>(//the second child in this column, will be a dropdown menu of the value choice
        requestFocusOnTap: true,//when the user hovers over a value in the dropdown menu list, there is more focus to it.
        label:Text("Choose a value"),//this represents the internal label of the dropdown menu
        onSelected: changeChoice,//when a value in the dropdown menu is selected, the previous value is changed to the selectedvalue
        dropdownMenuEntries: //insert dropdown menu stuff here.
        SelectedVal.listEntries,//listEntries was declared in the enum as a map.
        expandedInsets: EdgeInsets.only(),//this creates a left offset effectively increasing length of the DropDown Menu.
      )
      ...
      ,Padding(padding: EdgeInsets.only(left:40, top: 25, bottom: 10), child:Text("Enter the amount of entries you would like to see here(be sure to press Enter): ",style: TextStyle(fontSize: 17, color: Colors.purple),)),//this represents the title. Padding is placed to ensure that it is placed nicely into the UI
  ...
  child:TextField(
    ...
    decoration: InputDecoration( //Represents the label to be shown in the input field
      border: OutlineInputBorder(), //rounded rectangle border
      labelText: returnLabel2(), 
    ),
    maxLength: 5, //max amount of characters
  ...
  //)
    ...
    SizedBox(width:230, 
      ...
      Text(errorMessage, style: TextStyle(color: Colors.red))//this text will be shown if showExceedmessage is true.
    ...
    //,Padding(EdgeInsets.only, child: 
    //(left: 10, top: 0.05, child: Text("Hello\n"),)
    ,Padding(padding: EdgeInsets.only(left: 6, top: 6), child: SizedBox(height: 450, child: Text("Tables Page Instructions:\n\nIf you enter in all settings except for time, \nthe first x user inputted entries will be shown.\n\nIf you choose a time span, and you just put 1 entry, \nit will show the latest time value.\n\nIf you use a time value,\nand you enter in a number of entries\n which doesn't exceed the amount of entries for the timespan\n, the entries will be evenly spaced\n\nDateTime Format: Day Month Date Hours:Minutes:Seconds.Milliseconds Year")))
    
    ],)

    //ListView( //list view is a widget that is used to see how data changes in different blocks
      //c//hildren:_MyHomePageState.pausedData()
    //Text("cHEEERS MATE", style: TextStyle(fontSize: 63, color: const Color.fromARGB(255, 75, 34, 186)),),
    //)
  );
...
//this function is used to signout the user. The tab has the way for the user to signout.
  ...
  (
    ...
    (toolbarHeight: 35,//this adjusts the height of the appbar
      ...
      tabs:[//this instantiates a list of 3 tabs
    ...
    leading: IconButton(//leading means at the beginning
      ...
      , icon: Icon(Icons.logout_rounded)),//this is the sign-out button.
    ),
    body: 
...
class _SignInScreenState extends State<SignInScreen1> {//this extends the statefulwidget, to ensure changes in state are possible.
  ...
  Navigator.push( //push means go to other page
            context, //the context is the build context
            MaterialPageRoute(builder: (context) => const DefaultTabController(length: 3, child: TopTabs())), //this means it wants to go to the next page, which is going to be the tab controller(aka, the parts of the app that is not authentication).
  ...
  else if(e.code=='invalid-credential') {//invalid-credential is thrown whenever the email is in proper format(but still not found in Firebase), but the password is not. The email could still be incorrect
...
bool dontshowPass=true;//this is an indicator used whether or not to show the hidden password. If it is set to true, then that means the password will not be shown.
  ...
  //),
    ...
    //,Text("\n$currentEmail")]),
      ...
      //Column(child:Center(children: <Widget> [
...
class _SignUpScreenState extends State<SignUpScreen> {//this extends the statefulwidget, to ensure changes in state are possible(main changes in state will be text changes, and text deletion upon exiting).
  ...
  Future<void> signup() async {//this is used to connect the usersignin with firebase(firebase integration of authentication)
...
setState(() {
  ...
  {//this is the situation when both passwords do not equal each other, nor, do not both equal to empty characters
    ...
    message="Please make sure the passwords match. Sign-Up occurs only when both passwords match.";//Because the passwords do not match, sign in is not successful.
  });
  }
  ...
  setState(() {
    signupSuccessful=false;//bc the firebase did not match the record to a current record, the signin is not successful
    message=errorMessage;//the message now contains the value of errorMessage
  });
...
bool dontshowPass2=true;//same as dontshowPass, but this is for the verified password
  ...
  //) : null,
      ...
      //put this logic here.
      Visibility(visible: resetPassforSignUp, child:Text("\nIf you forgot your password, please click this button:", style:TextStyle(color:Colors.green))),
      ...
      //Visibility(visible: message=="The account already exists for that email", child: null,
      //children:<Widget>[Text("Hello")]) 
      
      //insert a conditionally dependent reset password button here.
  ...
  (
    ...
    Text("\nRemember your password? SignIn here: "),//Gives information on how to signin/make an account.
      ElevatedButton(//this is a prompt to the next screen
        child: Text("Sign-In"),//the button is called signin
        onPressed: () {
          //Navigator.pop( //pop means go to back to the signinpage
            //context, //the context is the build context
            //MaterialPageRoute(builder: (context) => const SignUpScreen()), //this means it wants to go to the next page, which is going to be the signupScreen.
  //);
